...
show_usage() {
    ...
    local invocation_command="${INCOVATION_CMD:-$(basename "$0")}"

    cat << EOF
Usage: ${invocation_command} [OPTION]
...
Examples:
  ${invocation_command}           # Sync documents & photos directories
  ${invocation_command} photos    # Sync only photos
EOF
}
